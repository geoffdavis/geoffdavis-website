...
- Bonus standalone knowledge: BIOS setup on these boxes is behind **Ctrl+F12** at the UGREEN splash screen, and **Ctrl+F1** on the main setup screen unlocks hidden advanced menus.
...
- **Ctrl+F12** at the UGREEN splash screen enters AMI BIOS setup.
- **Ctrl+F1** on the main setup screen unlocks hidden advanced menus — a whole set of chipset, power, and boot configuration pages that aren't visible by default.

This is worth knowing independent of the PXE question. The visible default menus are sparse; the Ctrl+F1 tier is where the real knobs live, including the network stack settings discussed below.
...
On the JetKVM: **Mount Drive**, pick the ISO, and use **CD/DVD mode** for images over 2.2GB (installer ISOs usually are). The DXP2800's firmware sees the virtual drive immediately, it shows up in the boot menu, and it boots. The KVM also sends the Ctrl+F12/Ctrl+F1 chords, so the entire loop — enter setup, pick boot device, install OS — works without touching the machine.
...
If you're trying to netboot one: use a USB stick, or better, an IP-KVM with virtual media. And remember **Ctrl+F12** for setup, **Ctrl+F1** for the hidden menus — you'll want both either way.
