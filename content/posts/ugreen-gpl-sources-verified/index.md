...
I want to be precise about this, because it isn't my win to claim. I never asked for any of it; my request was the ACL stack. And the kernel tree that contains these drivers has a `created_at` of 2026-04-03 — a month before I sent a word (more on why "created" isn't "published" in the timeline section below). So this hardware-driver release answers a *different* thread of GPL pressure than mine. The obvious candidate is the LED reverse-engineering community, who had spent years wanting exactly this source and every reason to ask for it. I can't point to a specific prior request, but I also don't need to: the drivers I didn't ask for showing up in a tree that predates my request is enough to say plainly that I wasn't the first or only person leaning on UGREEN here. Mine was one email among what looks like several, and the LED and fan drivers were somebody else's ask, answered before I arrived.
...
So the GitHub account has existed since August 2024, and the `created_at` timestamp on the big x86 kernel drop reads 2026-04-03 — a month *before* I sent anything. It's tempting to stop there and conclude the kernel source was available all along and I just missed it. But a creation date is not a publication date, and the difference matters here.

GitHub's `created_at` records when the repository *object* was created; it does **not** record when a repository was flipped from private to public, and the REST API exposes no field that does. A repo can sit private for months and carry a `created_at` from the day it was first initialized. When I wrote the original post in early May I searched — GitHub's own code search included — for these repos and for the module names (`ugacl_vfs`, `kmugacl`), and found nothing; nothing on UGREEN's site linked to the org either. My recollection is that the kernel tree was not publicly visible at that point. Two things line up with that: the repos covering the *userspace* half of my request, `btrfs-progs` and `Samba-4.17.12`, carry `created_at` dates of 2026-06-11, and GitHub's code search *still* doesn't index the kernel tree — behavior more consistent with a recent publication than a five-month-old public repo.

I can't prove the publication date from the API — nobody can, the field doesn't exist. But "created privately in April, made public around June with the rest" fits both the repo dates and my contemporaneous inability to find any of it, and it's a more honest reading than "public the whole time." If UGREEN can show the kernel tree was genuinely public before 2026-05-06, I'll correct this gladly — but the burden of that is on the timestamps they control, not on my failure to find something that a search engine indexing the whole of public GitHub also couldn't.
